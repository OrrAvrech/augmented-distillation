...
class CondEncoder(nn.Module):
    ...
    def __init__(self, input_size=1,
                 ...
                 log_dir='./log_dir'):
        """
            This model uses a Transformer Encoder Model
        """
    ...
    def gibbs_sampler(self, rounds, real_data, dim_orders, permute_dims=False, random_init=False):
        ...
        """
        # step 0: find device type e.g. cpu or gpu
    ...
    def sample_gibbs(self, data_loader, rounds, num_iter):
        joints = []
        for i in range(num_iter):
            for batch_idx, data in enumerate(data_loader):
                data_fetch = data.to(self.device)
                batch_joints, _ = self.gibbs_sampler(rounds=rounds,
                                                     real_data=data_fetch,
                                                     dim_orders=range(data_fetch.shape[1]))
                joints += list(batch_joints.numpy())
        return joints

    def sample_conditionals(self, data_loader, num_iter, mid=None):
        conditionals = []
        for i in range(num_iter):
            for batch_idx, data in enumerate(data_loader):
                data_fetch = data.to(self.device)
                with torch.no_grad():
                    ...
                    batch_conditionals = self.batch_feed_forward(data_fetch, mid=mid)
