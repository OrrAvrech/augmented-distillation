...
class TabularAutoML:
    def __init__(self, label, clip_prob_factor=1e-5, metric='f1_macro',
                 time_limit=60, presets='best_quality',
                 auto_stack=True):
        self.label = label
        self.predictor = TabularPredictor(self.label, eval_metric=metric)
        self.factor = clip_prob_factor
        # AutoGluon params
    ...
    def predict_proba(self, x):
        prob = self.predictor.predict_proba(x).to_numpy()
        prob = np.clip(prob, self.factor, 1.0)
        prob = prob / prob.sum(axis=1)[:, None]
        return prob
